...
- **真横(90度)は「見えるもの」で指示すれば出せる**(2026-07-29、`left`/`right` を追加)。
  「Tポーズの真横は構造的に破綻する」という旧記録は現行パイプラインでは覆った(破綻は
  起きない)が、**角度で指示する限り3/4止まり**になる。実測(被写体bboxの幅/高さ比。
  真横なら0.4前後、正面のTポーズは0.97〜1.00):
  角度指示(「左側から」「90度回転」「約75度」)0.56〜0.84 /
  45度画像を経由する段階回転 0.83〜0.96(悪化)/
  **「見えるもの」で指示 0.36〜0.52(到達)**。
  ただし「見えるもの」だけだと**腕が下りてTポーズが失われる**ため、腕の見え方も併記する。
  腕の書き方は2案を実測し、「手前腕は端面が円として胸に重なる」は投影として正しいが
  **約半数のseedで端面が別物の球になる**(青/黒/紺の球。材質を書いても再発)。
  **「両腕は強く短縮され肩と胴体の陰にほぼ隠れる」**に変えると人工物が消えた(採用)。
  残る不安定さは腕の短縮度合いが seed で変わること(0.38〜0.64)。
...
- **生成後の編集に「元画像を2枚目の参照として渡す」オプション**(`use_reference`、
  **既定OFF**、2026-07-28、ユーザー提案)。生成の2段目が [生成した正面, 元画像] で
  連鎖しているのと同じ手で、Edit も最大3枚まで参照を取れる。プロンプトで
  「1枚目=編集対象、2枚目=同じキャラの元画像」と書き分ける
  (`build_edit_prompt(with_reference=True)`)。**既定をOFFにしたのは実機A/Bの結果**:
  参照ありは元の見た目をよく思い出す(「元の髪型に戻す」で元のまとめ髪・前髪に
  近づいた)一方、**元画像のポーズ・背景まで引き戻す**。被写体bboxの幅/高さ比
  (Tポーズほど大きい)は 編集前 0.90 / 参照なし 0.89 / 参照あり 0.87・0.87・**0.27**
  で、3seed中1つでTポーズが壊れ自然な立ち姿へ戻った(別seedでは背景が市松模様)。
  衣装修正でも参照ありは数値上は良い(インナー露出 1.3% vs 6.1%)のに
  **目視ではボレロが膝丈のワンピースへ伸びていた**。**数値だけで判定しないこと**。

## アップスケール(2048、2026-07-28)

- `core/upscale.py` が **Real-ESRGAN x2**(`ai-forever/Real-ESRGAN` の
  `RealESRGAN_x2.pth`、64MB)を **spandrel** 経由で読む。spandrel は comfy-env に
  既存なので**依存は増えていない**。`DS_UPSCALE_MODEL` でローカルの重みに差し替え可。
- **拡散モデル(Qwen-Image-Edit)で2048を再生成する案は採らなかった**: 解像度は
  上がっても再生成なので細部が書き換わる(このアプリが繰り返し戦ってきた髪型・衣装の
  ドリフトがそのまま起きる)。3D入力用途では「同じ絵を精細にする」ことが要件なので、
  決定論的なGAN系アップスケーラが正しい選択。
- 実測: 1024→2048 が **2.0秒 / ピークVRAM 4.09GB**(RRDBNet 16.7M params、fp32、
  タイル分割なしの1パス)。同一スケール(2048同士)の比較では
  ラプラシアン分散 Lanczos 11.0 に対し **51.9**、最大エッジ勾配 666 に対し **1017**
  (元1024は 108.0 / 960)。構造は保存される(2048版を1024へ戻して元と比較して
  PSNR 38.0dB・相関 0.99924。GANなので画素レベルでは細部を合成しており完全一致では
  ないが、髪型・衣装・輪郭は動かない)。
  **測定の注意**: 最初は「ラプラシアン分散 36.9 vs 15.0」と記録したが、これは別画像・
  別条件の値だった。加えて「ガウシアン差分の高周波エネルギー」で測ると
  Lanczos と ESRGAN が同値(0.99)になり、シャープさの差を捉えられない
  (固定σでは 1〜4px 帯しか見ないため)。**シャープさはラプラシアン分散か
  エッジ勾配で、同一スケール同士で比較すること**。
- **1024の情報量が2048分に増えるわけではない**(画素あたりの情報密度は元より低い)。
  ネイティブ高解像度生成(`DS_TPOSE_SIZE` を上げる)は未検証で、モデルが約1MP前提の
  学習であること・再生成によるドリフトのリスクがある。
- 透過版は**拡大後に切り抜きを作り直す**(2048で切り抜いた方が輪郭が精細)。
  元画像を `/edit`・`/undo` で変えたら `_clear_upscaled()` が古い2048版を捨てる
  (取り違え防止)。
