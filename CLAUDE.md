...
- **左右の向きは「視点名」と競合する / 右向きは鏡像で作る**(2026-07-29、ユーザー報告
  「左右が同じ方向」)。① 視点句に `left side profile view` のような**視点名**を書くと、
  モデル側の解釈と「画面左を向く」という指示が競合して**左右が反転する**(実測 1/3 で反転)。
  視点名をやめて**向きだけを言う**と 3/3 で指示どおりになる。② ただし**右向きは言葉では
  出せない**: 「画面右を向く」「背中が画面左」「腕が画面右へ伸びる」の3文言 × 3seed = 9回中
  **成功1回**。モデルに「横顔は左向き」という強いバイアスがある。そこで `right` は
  **参照画像を鏡像にして左向きを生成し、出力を左右反転して戻す**(`tpose/jobs.py` の
  `mirrored`)。鏡像キャラの左側面 = 元キャラの右側面なので、**左右非対称の意匠も
  正しい側に出る**(単なる左右反転コピーとは違う)。実測で left/right とも期待どおりの
  向きになった。**45度は左のみ提供する**(モデルが45度の左右を区別できず
  `front_left_45` と `front_right_45` で同じ絵が出ていたため、ユーザー判断で右を削除)。
- **真横(`left`/`right`)では腕を前方へ出す**(2026-07-29、ユーザー判断)。
  Tポーズのまま真横は**描けない**(手前の腕がカメラを指す極端な短縮)。実測では
  **胴体は完璧に真横まで回り込むのに腕だけが壊れる**: ぬいぐるみ(腕が短い)は
  胴体の陰に隠せて成立するが、**人物(腕が長い)は腕が長い管・棒になって画面外へ伸びる**。
  angles LoRA でも同じで、**charsheet の真横が綺麗なのはプロンプトが
  "neutral standing pose"(腕を下ろした姿勢)だから**(LoRAを使っても「Tポーズ維持」と
  書くと回り込みが浅くなる: 0.59 -> 0.75)。腕を下ろすと胴体の側面が隠れるため、
  **腕を前方へ水平に出す**方式を採用した(胴体側面が見え、極端な短縮も不要。人物・
  ぬいぐるみとも安定)。**このビューだけ他と腕の姿勢が違う**のは意図的。
  横向きの靴は、腰・膝・足首を投影上で揃え、手前の靴1足だけを側面表示する専用句を
  プロンプト末尾へ置く。胴体だけが真横でも足が3/4のままだと手前・奥の靴が両方見えるため。
  **ゴースト顔の罠**: 視点句に「鼻が画面端を向く/見える目と耳は片方だけ」と顔のパーツ語を
  入れると、アニメ絵で**巨大な顔・目・耳が背景に湧く**(ユーザー報告)。回り込みの
  手がかりは腕の前方指示が担うので、顔のパーツ語は視点句から外すこと。
- (旧記録)**真横は「見えるもの」で指示すると回り込む**(角度指示では3/4止まり)。
...
- 透過版に残った白は `core/alpha_refine.py` で後処理できる。自動処理は既存の透明領域に
  接する近似白だけを除去し、髪で囲まれた白はUIのクリック座標から同系色の連結領域だけを
  除去する。白目や服の光沢を守るため、囲まれた白を自動で一括削除しない。
