...
(正面 / 背面 / 左真横 / 右真横 / 左前45度)を生成する専用サーバ。
...
極端な短縮になり**描けない**(胴体は綺麗に回り込むのに、腕だけが長い管・棒になって
画面外へ伸びる)。腕を下ろすと綺麗になるが、今度は腕が胴体の側面を隠してしまう。
そこで**真横では腕を前方へ水平に出す**(胴体の側面シルエットが隠れず、極端な短縮も
...
なお**45度ビューは左のみ**提供する(モデルが45度の左右を区別できず、左右で同じ絵が
